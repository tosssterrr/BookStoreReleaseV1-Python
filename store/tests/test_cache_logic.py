...
from store.models import User, Category, Author, Book, UserBookRelation
...
class SetRatingTestCase(TestCase):
    def setUp(self):
        ...
        UserBookRelation.objects.create(user=self.user_1, book=self.book_1, like=True, rate=7)
        UserBookRelation.objects.create(user=self.user_2, book=self.book_1, like=True, rate=9)
        UserBookRelation.objects.create(user=self.user_3, book=self.book_1, like=True, rate=6)
    ...
    def test_ok(self):
        ...
        self.assertEqual('7.33', str(self.book_1.rating))
